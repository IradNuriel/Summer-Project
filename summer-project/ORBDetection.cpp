...
ORBDetection::ORBDetection(const cv::Mat& img) {
	this->minHessian = 1200;
	this->detector = cv::ORB::create(minHessian);
	this->imageSet = std::vector<cv::Mat>();
...
ORBDetection::ORBDetection(const ORBDetection& other) {
	this->minHessian = other.minHessian;
	this->detector = cv::Ptr<cv::ORB>(other.detector);
	this->imageSet = std::vector<cv::Mat>(other.imageSet);
}

std::vector<cv::KeyPoint> ORBDetection::getFeaturesOfImage(int i) {//using opencv ORB features detection
	std::vector<cv::KeyPoint> keypoints;
	this->detector->detect(this->imageSet[i], keypoints);
	return keypoints;
}

const ORBDetection& ORBDetection::addImageToSet(const cv::Mat img) {//adding image to the data set
	this->imageSet.push_back(img);
	return *this;
}

const ORBDetection& ORBDetection::removeImageFromSet(int i) {//removing image from the data set
	this->imageSet.erase(this->imageSet.begin() + i);
	return *this;
}

void ORBDetection::showImageWithKeyPoints(int i) {//show the i'th image with its keypoints, also save it to a file
